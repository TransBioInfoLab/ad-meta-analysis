...
```{R}
files <- dir(pattern = paste0(".*neuro"),
             recursive = T,
             full.names = TRUE,
             ignore.case = T)
...
pheno_df <- readRDS(
  dir("DATASETS/LONDON/",pattern = "Neuron",recursive = TRUE,full.names = TRUE)
)
samples <- readRDS(
  paste0(dir.result, "London_samples.RDS")
)
...
```{R gasparoni_singleCpG, eval = TRUE}
### Import datasets
beta_mat <- readRDS(
...
3. meta analysis
4. add annotation
5. Delete sig DMRs with cross-hybridizing or smoking probes 
6. Compare with sig DMRs from main analysis 
...
Gasparoni <- preMeta(cohort = "Gasparoni") 
dim(Gasparoni)
...
5. Compare with sig CpGs from main analysis  
