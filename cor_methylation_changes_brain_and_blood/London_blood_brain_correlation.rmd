...
# Find overlaps with Hannon's paper
...
supp7 <- read_xlsx(
  paste0(data.BECon, "Hannon_Epigenetics_2015_Copy of SupplementaryTables.xlsx"),
  sheet = "Supplementary Table 7",
  skip = 1
)

supp7_cpg_char <- supp7$...1 #n=1813

IntersectCpGs_ls <- lapply(seq_len(nrow(main_dmrs)), function(row){

  regionName <- as.character(main_dmrs$inputRegion)[row]

  cpgs_sig <- GetCpGsInRegion(regionName, arrayType = "450k")

  cpgs_intersect <- intersect(cpgs_sig, supp7_cpg_char)

  cpgs_intersect_char <- paste0(cpgs_intersect, collapse = "; ")

  data.frame(
    inputRegion = regionName,
    nIntersectCpGs = length(cpgs_intersect),
    IntersectCpGs = cpgs_intersect_char,
    stringsAsFactors = FALSE
  )

})

IntersectCpGs_df <- do.call(rbind, IntersectCpGs_ls)

table(IntersectCpGs_df$nIntersectCpGs)
#   0
# 119

### Conclusion: no overlaps
...
IntersectCpGs_ls <- lapply(seq_len(nrow(main_cpgs)), function(row){

  cpgName <- as.character(main_cpgs$cpg)[row]

  cpgs_intersect <- intersect(cpgName, supp7_cpg_char)
...
## with website(need to manually check each cpg)  
