...
Filtering samples with bisulfiteConversion lower than 88.
...
load(file.path(data.dir.pca,"qtestPerm.rda"))
```
...
matboxplot(beta_mat,
           groupFactor =  pheno_df$stage %>%  as.factor(),
           xaxt = "n",
           main = "Beta Values in Gasparoni cohort (autosomes) - before normalization")
legend('bottom',
       paste0("stage ",  levels(pheno_df$stage %>%  as.factor())),
       col = c(1:7), 
       lty = 1, 
...
matboxplot(betaQN,
           groupFactor =  pheno_df$stage %>%  as.factor(),
           xaxt = "n",
           main = "Beta Values in Gasparoni cohort (autosomes) - after normalization")
```
