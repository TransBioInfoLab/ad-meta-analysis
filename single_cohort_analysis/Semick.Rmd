...
data.dir.raw.idat <- file.path(data.dir.raw, "IDATs/")
...
data.dir.validation <- file.path(data.dir,"/step12_validation/") 
...
geo.data <- getGEO(GEO = "GSE125895",destdir = data.dir.raw)

idat.files <- c(geo.data$GSE125895_series_matrix.txt.gz$supplementary_file %>% as.character(),
                geo.data$GSE125895_series_matrix.txt.gz$supplementary_file.1 %>% as.character())


library(R.utils)
plyr::l_ply(idat.files, function(f){
  fout <- paste0(data.dir.raw.idat,basename(f))
  if(!file.exists(fout)) downloader::download(f,fout)
...
saveRDS(RGSet, paste0(data.dir.raw, "RGSet.RDS"))

phenoData <- pData(geo.data$GSE125895_series_matrix.txt.gz)
phenoData$Array <- stringr::str_sub(phenoData$supplementary_file, -29, -20)

saveRDS(phenoData, paste0(data.dir.raw, "GSE125895_pheno.RDS"))
write.csv(phenoData, paste0(data.dir.raw, "GSE125895_pheno.csv"))

SampleSheet <- phenoData[, c("geo_accession", "Array", "sentrix_position:ch1")]
colnames(SampleSheet) <- c("Sample_Name", "Slide", "Array")

### Create basename in SampleSheet file to locate idat files in next script
SampleSheet$Basename <- paste0(
  "./", SampleSheet$Sample_Name, "_",
  SampleSheet$Slide, "_",
  SampleSheet$Array
)

### Save SampleSheet file
write.csv(SampleSheet, paste0(data.dir.raw, "SampleSheet.csv"), row.names = FALSE)
```
...
```{R, eval = FALSE}
RGSet <- readRDS(file = paste0(data.dir.raw, "RGSet.RDS"))
...
2. variables -- state, age, sex

- subset methylation data to match subjects in pheno data 

- Input: GSE125895_pheno.csv, RGSet_bsfiltered.rda

- Output: pheno_df.RDS, beta_mat.RDS

```{r data_procesing_libs}
### Read in phenotype data
phenoRaw_df <- read.csv(paste0(data.dir.raw, "GSE125895_pheno.csv")) 

### Subset rows and columns
pheno_df <- phenoRaw_df[
  (phenoRaw_df$source_name_ch1 == "DLPFC") &
    !is.na(phenoRaw_df$disease.state..dx..ch1),
  c("geo_accession", "Array",
    "age.ch1", "Sex.ch1", "disease.state..dx..ch1")
  ]

pheno_df$disease.state..dx..ch1 <- ifelse(
  pheno_df$disease.state..dx..ch1 == "Alzheimer", 1, 0
)

### Rename vars
colnames(pheno_df) <- c(
  "sample", "slide", "age.brain", "sex", "status"
)
...
pheno_df$sex <- as.character(pheno_df$sex)
pheno_df$status <- as.character(pheno_df$status)

dim(pheno_df) #dim: 68 5
```

```{R}
##### 3. Subset RGSet ##########################################################
load(file = paste0(data.dir.bsfilter, "RGSet_bsfiltered.rda"))
colnames(RGSet) <- grep("GSM",unlist(stringr::str_split(colnames(RGSet),"_")),value = TRUE)
### Subset RGSet based on pheno data
RGSet <- RGSet[, pheno_df$sample] #dim: 622399 68
...
saveRDS(beta_mat, paste0(data.dir.clinical.filter, "beta_mat.RDS"))
...
beta_mat <- readRDS(paste0(data.dir.clinical.filter, "beta_mat.RDS"))
dim(beta_mat)

beta_mat <- beta_mat[grep("cg",rownames(beta_mat)),]
dim(beta_mat)
nb.cg.probes <- nrow(beta_mat)
```
...
dim(betaFiltered_mat)
nb.probes.cg.dmrcate <- nrow(betaFiltered_mat)
```
...
```{R}
pheno_df$status <- factor(pheno_df$status)
...
legend('top',
       paste0("status ", levels(pheno_df$status)),
...
matboxplot(beta_mat,
           groupFactor = pheno_df$status,
           xaxt = "n",
           main = "Beta Values in Semick cohort (autosomes) - before normalization")
...
matboxplot(betaQN,
           groupFactor = pheno_df$status,
           xaxt = "n",
           main = "Beta Values in Semick cohort (autosomes) - after normalization")
...
saveRDS(betaQN_BMIQ, paste0(data.dir.probes.normalization, "Semick_QNBMIQ.RDS"))
```

```{R, include = FALSE}
betaQN_BMIQ <- readRDS(paste0(data.dir.probes.normalization, "Semick_QNBMIQ.RDS"))
```
...
##### 0.Import datasets ########################################################
beta_mat <-
  readRDS(paste0(data.dir.probes.normalization, "Semick_QNBMIQ.RDS")) #dim: 437713 142
...
betaOrd_mat <- OrderDataBySd(beta_mat) #dim: 399426 68

mOrd_mat <- OrderDataBySd(mvalue_mat)  #dim: 399426 68


betaOrd_matPlot <- betaOrd_mat[, both_df$sample] #dim: 399426 68
mOrd_matPlot <- mOrd_mat[, both_df$sample]       #dim: 399426 68
identical(both_df$sample, colnames(betaOrd_matPlot))
identical(both_df$sample, colnames(mOrd_matPlot))

expSorted_mat = betaOrd_mat #dim: 399426 68
...
readr::write_csv(d, paste0(data.dir.pca, "Semick_PCs_usingBetas.csv"))
```
...
saveRDS(betaQN_BMIQ_PCfiltered, paste0(data.dir.pca, "Semick_QNBMIQ_PCfiltered.RDS"))

pheno_df <- pheno_df[pheno_df$sample %in% rownames(noOutliers),] #dim: 141 7
...
df.probes
save(df.samples,df.probes,file = file.path(data.dir.table, "Semick_table.rda"))
```
...
pfc <- readRDS(paste0(data.dir.pca, "Semick_QNBMIQ_PCfiltered.RDS")) #dim: 433656 59
...
## Find coMethylated regions using residuals
...
### Get Residuals to find coMethylated Regions

```{R, eval = FALSE}
##### 1. Import datasets #######################################################
beta_mat <- readRDS(grep("QNBMIQ_PCfiltered",dir(data.dir.pca,full.names = T),ignore.case = T,value = T)) 
...
saveRDS(
  ...
  paste0(data.dir.residuals, 
         "Semick_QNBMIQ_PCfiltered_mvalResiduals.RDS")
)
```
...
saveRDS(
  coMeth_ls,
  paste0(data.dir.residuals, 
         "Semick_residuals_cometh_input_ls.RDS")
)
```
...
pheno_df$status <- as.numeric(pheno_df$status)
...
semick_dmrs <- readRDS(
    dir(data.dir.median,pattern =  "Semick_linear_df.RDS",full.names = T, ignore.case = T)
) #dim: 40176 8

### select the most significant cluster for each input region  
semick_selected <- semick_dmrs %>%
  group_by(Semick_inputRegion) %>%
  filter(Semick_pVal == min(Semick_pVal)) #dim: 40001 8

### subset validation datasets with significant input regions in meta analysis
semick_metaSig <- semick_selected[
  match(main_dmrs$inputRegion, semick_selected$Semick_inputRegion),
]
semick_metaSig <- semick_metaSig[
  !is.na(semick_metaSig$Semick_inputRegion),
]
dim(semick_metaSig)
# [1] 119   8

### calculate validation fdr
semick_metaSig$Semick_validation_fdr <- p.adjust(
  semick_metaSig$Semick_pVal, method = "fdr"
)

### add annotation
semick_metaSig_withAnnot <- merge(
  main_dmrs[,1:5], semick_metaSig,
  by.x = "inputRegion",
  by.y = "Semick_inputRegion",
  sort = FALSE
)
write.csv(
  semick_metaSig_withAnnot,
  paste0(data.dir.validation, "validation_semick.csv"),
  row.names = FALSE
)
```
...
### Import datasets
beta_mat <- readRDS(
    paste0(data.dir.pca, "Semick_QNBMIQ_PCfiltered.RDS")
) 
pheno_df <- readRDS(
 paste0(data.dir.neuron, "pheno_withNeuronProp_df.RDS")
)
...
write.csv(
  results_ordered_df,
  paste0(data.dir.single.cpg.pval, "Semick_single_cpg_linear_df.csv"),
  row.names = FALSE
)
```
```{R}
...
semick_cpgs <- read.csv(
    paste0(data.dir.single.cpg.pval, "Semick_single_cpg_linear_df.csv")
) #dim: 450793 4

### subset validation datasets with significant cpgs in meta analysis
semick_metaSig <- semick_cpgs[
  match(
      as.character(main_cpgs$cpg),
      as.character(semick_cpgs$cpg)
  ),
]
semick_metaSig <- semick_metaSig[
  !is.na(semick_metaSig$cpg),
]
dim(semick_metaSig)
# [1] 3751    4
...
### calculate validation fdr
semick_metaSig$semick_validation_fdr <- p.adjust(
  semick_metaSig$pValue, method = "fdr"
)

### add annotation
semick_metaSig_withAnnot <- merge(
  main_cpgs[,1:8], semick_metaSig,
  by.x = "cpg",
  by.y = "cpg",
  sort = FALSE
)
...
write.csv(
  semick_metaSig_withAnnot,
  paste0(data.dir.validation, "validation_single_cpg_semick.csv"),
  row.names = FALSE
)
```
