...
data.dir.validation <- file.path(data.dir,"/step12_validation/") #Lanyu added
...
geo.data <- getGEO(GEO = "GSE57360",destdir = data.dir.raw)

assayData <- geo.data$GSE57360_series_matrix.txt.gz@assayData$exprs
saveRDS(assayData, paste0(data.dir.raw, "GSE57360_assay.RDS"))
write.csv(assayData, paste0(data.dir.raw, "GSE57360_assay.csv"))

phenoData <- geo.data$GSE57360_series_matrix.txt.gz@phenoData@data
saveRDS(phenoData, paste0(data.dir.raw, "GSE57360_pheno.RDS"))
write.csv(phenoData, paste0(data.dir.raw, "GSE57360_pheno.csv"))
```
...
1. Alzheimer's disease and control
2. variables -- geo_accession, age, sex, condition
- subset methylation data to match subjects in pheno data (n = 12)

- Input: GSE57360_pheno.csv, GSE57360_assay.RDS

- Output: pheno_df.RDS, beta_mat.RDS

```{r data_procesing_libs}
### Read in phenotype data
phenoRaw_df <- read.csv(paste0(data.dir.raw, "GSE57360_pheno.csv")) #dim: 29 42

### Subset rows and columns
pheno_df <- phenoRaw_df[
  phenoRaw_df$condition.ch1 %in% c(
    "Alzheimer's disease", "Control", "Alzheimer Disease"),
  c("geo_accession", "age.ch1", "gender.ch1", "condition.ch1")
]

pheno_df$condition.ch1 <- ifelse(
  pheno_df$condition.ch1 == "Control", 0, 1
)

### Rename vars
colnames(pheno_df) <- c(
  "sample", "age.brain", "sex", "status"
)
...
beta_mat <- readRDS(paste0(data.dir.raw, "GSE57360_assay.RDS"))
...
matboxplot(beta_mat,
           groupFactor = pheno_df$status,
           xaxt = "n",
           main = "Beta Values in SanchexMut cohort (autosomes) - before normalization")
...
matboxplot(betaQN,
           groupFactor = pheno_df$status,
           xaxt = "n",
           main = "Beta Values in SanchexMut cohort (autosomes) - after normalization")
...
saveRDS(betaQN_BMIQ, paste0(data.dir.probes.normalization, "SanchexMut_QNBMIQ.RDS"))
```

```{R, include = FALSE}
betaQN_BMIQ <- readRDS(paste0(data.dir.probes.normalization, "SanchexMut_QNBMIQ.RDS"))
```
...
```{R}
##### 0.Import datasets ########################################################
beta_mat <- readRDS(paste0(data.dir.probes.normalization, "SanchexMut_QNBMIQ.RDS")) #dim: 437713 142
...
readr::write_csv(d, paste0(data.dir.pca, "SanchexMut_PCs_usingBetas.csv"))
```
...
library(ggrepel)
byStage <- plotPCA(
  dataset = "SanchexMut: beta values",
...
saveRDS(betaQN_BMIQ_PCfiltered, paste0(data.dir.pca, "SanchexMut_QNBMIQ_PCfiltered.RDS"))
...
df.probes
save(df.samples,df.probes,file = file.path(data.dir.table, "SanchexMut_table.rda"))
```
...
- Input: QNBMIQ_PCfiltered.RDS, pheno_df.RDS
...
pfc <- readRDS(paste0(data.dir.pca, "SanchexMut_QNBMIQ_PCfiltered.RDS")) #dim: 433656 59
...
lmF <- function(mval){
  fitE <- lm(
    as.numeric(mval) ~  age.brain + sex + prop.neuron,
    data = pheno_df,
...
saveRDS(
  ...
  paste0(data.dir.residuals, 
         "SanchexMut_QNBMIQ_PCfiltered_mvalResiduals.RDS")
)
```

### Find co-methylated regions
```{R comethDMR, eval = FALSE}
### Import datasets
...
saveRDS(
  coMeth_ls,
  paste0(data.dir.residuals, 
         "SanchexMut_residuals_cometh_input_ls.RDS")
)
```
...
covariates_char <- c("age.brain", "sex", "prop.neuron")
...
sanchexMut_dmrs <- readRDS(
   dir(data.dir.median,pattern = "SanchexMut_linear_df.RDS",ignore.case = TRUE,full.names = TRUE)
) #dim: 40224 8

### select the most significant cluster for each input region 
sanchexMut_selected <- sanchexMut_dmrs %>%
  group_by(SanchexMut_inputRegion) %>%
  filter(SanchexMut_pVal == min(SanchexMut_pVal))

### subset validation datasets with significant input regions in meta analysis
sanchexMut_metaSig <- sanchexMut_selected[
  match(
      as.character(main_dmrs$inputRegion),
      as.character(sanchexMut_selected$SanchexMut_inputRegion)
  ),
]
sanchexMut_metaSig <- sanchexMut_metaSig[
  !is.na(sanchexMut_metaSig$SanchexMut_inputRegion),
]
dim(sanchexMut_metaSig)
# [1] 119   8

### calculate validation fdr
sanchexMut_metaSig$SanchexMut_validation_fdr <- p.adjust(
  sanchexMut_metaSig$SanchexMut_pVal, method = "fdr"
)

### add annotation
sanchexMut_metaSig_withAnnot <- merge(
  main_dmrs[,1:5], sanchexMut_metaSig,
  by.x = "inputRegion",
  by.y = "SanchexMut_inputRegion",
  sort = FALSE
)

write.csv(
  sanchexMut_metaSig_withAnnot,
  paste0(data.dir.validation, "validation_sanchexMut.csv"),
  row.names = FALSE
)
```
...
### Import datasets
beta_mat <- readRDS(
    paste0(data.dir.pca, "SanchexMut_QNBMIQ_PCfiltered.RDS")
) 
pheno_df <- readRDS(
...
write.csv(
  results_ordered_df,
  paste0(data.dir.single.cpg.pval, "SanchexMut_single_cpg_linear_df.csv"),
  row.names = FALSE
)
```
```{R}
...
sanchexMut_cpgs <- read.csv(
    paste0(data.dir.single.cpg.pval, "SanchexMut_single_cpg_linear_df.csv")
) #dim: 450793 4

### subset validation datasets with significant cpgs in meta analysis
sanchexMut_metaSig <- sanchexMut_cpgs[
  match(
      as.character(main_cpgs$cpg),
      as.character(sanchexMut_cpgs$cpg)
  ),
]
sanchexMut_metaSig <- sanchexMut_metaSig[
  !is.na(sanchexMut_metaSig$cpg),
]
dim(sanchexMut_metaSig)
# [1] 3751    4
...
### calculate validation fdr
sanchexMut_metaSig$SanchexMut_validation_fdr <- p.adjust(
  sanchexMut_metaSig$pValue, method = "fdr"
)

### add annotation
sanchexMut_metaSig_withAnnot <- merge(
  main_cpgs[,1:8], sanchexMut_metaSig,
  by.x = "cpg",
  by.y = "cpg",
  sort = FALSE
)
...
write.csv(
  sanchexMut_metaSig_withAnnot,
  paste0(data.dir.validation, "validation_single_cpg_sanchexMut.csv"),
  row.names = FALSE
)
```
