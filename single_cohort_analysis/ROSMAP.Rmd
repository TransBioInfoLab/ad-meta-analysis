...
library(quantro)
library(doParallel)
qtestPerm <- quantro(object = betaSet[complete.cases(betaSet),],
                     groupFactor = phenoData$braaksc,
                     B = 1000)
save(qtestPerm,file = file.path(data.dir.probes.normalization,"quantro.rda"))
```

```{R}
# save(qtestPerm,file = file.path(dir.dnam,"quantro.rda"))
load(file.path(data.dir.probes.normalization,"quantro.rda"))
```
...
```{R}
## 4. Density plot by disease stage
phenoData$braaksc <- factor(phenoData$braaksc)

matdensity(object = betaSet,
           groupFactor = phenoData$braaksc,
           xlab = " ", ylab = "density",
           main = "Beta Values", brewer.n = 8, brewer.name = "Dark2")

legend('top',
       paste0("stage ", levels(phenoData$braaksc)),
...
matboxplot(betaSet,
           groupFactor = phenoData$braaksc,
           xaxt = "n",
           main = "Beta Values in ROSMAP cohort (autosomes) - before normalization")
...
matboxplot(betaQN,
           groupFactor = phenoData$braaksc,
           xaxt = "n",
           main = "Beta Values in ROSMAP cohort (autosomes) - after normalization")
legend('bottom',
       paste0("stage ", levels(phenoData$braaksc)),
       col = c(1:7), lty = 1, lwd = 3, cex = 0.70)
```
...
dim(phenoData)
ggpubr::gghistogram(phenoData$braaksc,bins = 10)
```
