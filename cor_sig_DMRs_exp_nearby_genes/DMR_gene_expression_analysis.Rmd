...
tab <- plyr::adply(
  ...
  .fun = function(row) {
    tryCatch({
      ...
      results.all.pval <- summary(results.all)$coefficients[-1, 4, drop = F] %>% t %>% as.data.frame()
      results.all.estimate <-  summary(results.all)$coefficients[-1, 1, drop = F] %>% t %>% as.data.frame()
      colnames(results.all.pval) <- paste0("all_pval_", colnames(results.all.pval))
      colnames(results.all.estimate) <- paste0("all_estimate_", colnames(results.all.estimate))
...
For each CpG significantly associated with Braak stage, we looked for genes within $250Kbp$ and correlated DNAm with gene expression with the following model, for cases and controls separately:
