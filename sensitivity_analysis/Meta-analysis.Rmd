...
estimation_of_inflation <- function(data){
  ...
  print("Inflation factor before correction")
  ...
  # inflation factor
  print("Inflation factor after correction")
  print(inflation(bc))
  print(bias(bc))
  ...
  print("Correlation between pValue and pValue.bacon")
  print(
    cor.test(
      data.with.inflation$pValue, 
      data.with.inflation$pValue.bacon, 
      method = "spearman"
    )
  )
...
cohort <- "GASPARONI"
write.csv(
    est.inflation$data.with.inflation,
...
for(i in results.files){
    data <- readr::read_csv(i,col_types = readr::cols())
...
```{R}
readr::read_csv(
  file.path(dir.data, "meta_analysis_single_cpg_sig_no_crossHyb_smoking_df_bacon.csv"),
...
```{R}
overlapping.results <-  readr::read_csv(
    file.path(dir.result.comp, "meta_analysis_no_crossHyb_smoking_ov_comb_p.csv"),
    col_types = readr::cols()
)
